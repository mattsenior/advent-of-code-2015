(ns adventofcode-devcards.day-3
  (:require
...
(defn instructions->pos-counts [in]
  (->> in
       (reduce
        (fn [acc [santa-idx v]]
          (let [santa (get acc santa-idx {:current-pos [0 0] :trace {[0 0] 1}})
                next-pos (apply-movement (:current-pos santa) v)
                santa (update-in santa [:trace next-pos] inc)
                santa (assoc santa :current-pos next-pos)]
            (assoc acc santa-idx santa)))
        {})
       vals
       (map :trace)
       (apply merge-with +)
...
(defn instructions->house-count
  ([in]
   (instructions->house-count 1 in))
  ([santa-count in]
   (->> in
        split
        (map vector (cycle (range santa-count)))
        instructions->pos-counts
        #_count
        )))
...
(deftest part-1-tests
  (testing
    ...
    (is (= 2 (instructions->house-count ">"))))
  (testing
      "^>v< delivers presents to 4 houses in a square, including twice to the house at his starting/ending location."
    (is (= 4 (instructions->house-count "^>v<"))))
  (testing
    "^v^v^v^v^v delivers a bunch of presents to some very lucky children at only 2 houses."
    (is (= 2 (instructions->house-count "^v^v^v^v^v")))))

(defcard part-1-result
  (instructions->house-count input))

(deftest part-2-test
  (testing "^v delivers presents to 3 houses, because Santa goes north, and then Robo-Santa goes south."
    (is (= 3 (instructions->house-count 2 "^v"))))
  (testing "^>v< now delivers presents to 3 houses, and Santa and Robo-Santa end up back where they started."
    (is (= 3 (instructions->house-count 2 "^>v<"))))
  (testing "^v^v^v^v^v now delivers presents to 11 houses, with Santa going one direction and Robo-Santa going the other."
    (is (= 11 (instructions->house-count 2 "^v^v^v^v^v")))))

(defcard part-2-result
  (instructions->house-count 2 input))
