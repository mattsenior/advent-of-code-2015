(ns adventofcode-devcards.day-6
  (:require
...
(defn line->actions [x]
  ...
  (let [m (re-matches #"^(turn on|turn off|toggle) (\d+),(\d+) through (\d+),(\d+)" x)
        [_ action x1 y1 x2 y2] m
        [x1 y1 x2 y2] (map int [x1 y1 x2 y2])]
    [action [x1 y1] [x2 y2]]))

(defn lines->actions [lines]
  (->> lines clojure.string/split-lines (map line->actions)))
...
(defn count-lights [d lines]
  (let [cells (cells d)
        actions (lines->actions lines)]
    (reduce
     (fn [lights-on [x y]]
       (let [on? (->> actions
                      (filter (fn [[action [x1 y1] [x2 y2]]]
                                (and (>= x x1)
                                     (<= x x2)
                                     (>= y y1)
                                     (<= y y2))
                                ))
                      (reduce (fn [acc2 [action [x1 y1] [x2 y2]]]
                                (condp = action
                                  ON true
                                  OFF false
                                  TOGGLE (not acc2)
                              ...
                              false))]
         (if on? (inc lights-on) lights-on)))
       0
       cells)))


(defn cell->light-level
  [actions [x y]]
  (let [xf (comp (filter (fn [[action [x1 y1] [x2 y2]]]
                           (and (>= x x1)
                                (<= x x2)
                                (>= y y1)
                                (<= y y2))))
        ...
        rf (fn
             ...
             ([light-level action]
              (when action
                (condp = action
                  ON (inc light-level)
                  OFF (max 0 (dec light-level))
                  TOGGLE (+ 2 light-level)))))]
    (transduce xf rf 0 actions)))

(defn total-light-level [d lines]
  (let [actions (lines->actions lines)]
    (->> (cells d)
         (map (partial cell->light-level actions))
         (reduce + 0))))


(deftest part-1-tests
  (testing "Part 1"
    (is (= 2 (count-lights 10 "turn on 0,0 through 1,1\ntoggle 0,1 through 1,1")))))

(deftest part-2-tests
  (testing "Part 2"
    (is (= 1 (total-light-level 10 "turn on 0,0 through 0,0")))
    (is (= 2000000 (total-light-level 1000 "toggle 0,0 through 999,999")))))
...
#_(defcard part-1-result
  (count-lights 1000 input))

#_(defcard part-2-result
  (total-light-level 1000 input))
